...
void plotKDs(const int iKD, bool withSMDcut = false){
...
cout << file_tdata << endl;
        ...
        for (int k = 0; k < nKDs; k++){
/*
            tsig [j]->SetBranchAddress(KDlist[k], &KD_vars[k]);
            tggzz[j]->SetBranchAddress(KDlist[k], &KD_vars[k]);
            tqqzz[j]->SetBranchAddress(KDlist[k], &KD_vars[k]);
            tz   [j]->SetBranchAddress(KDlist[k], &KD_vars[k]);
*/
        };
...
#ifdef HeshyData
        cFill_data.ReplaceAll("D_bkg_kin","D_bkg_kin_Heshy");
#endif
        if(!withSMDcut) tdata[j]->Draw(cFill_data,"(ZZMass<100.0&&ZZMass>=80.0)");
#ifdef HeshyData
        else tdata[j]->Draw(cFill_data,"(ZZMass<100.0&&ZZMass>=80.0 && D_bkg_kin_Heshy>0.5)");
#else
        else tdata[j]->Draw(cFill_data,"(ZZMass<100.0&&ZZMass>=80.0 && D_bkg_kin>0.5)");
#endif
        ...
        if(!withSMDcut) tsig[j]->Draw(cFill_sig,"MC_weight * (ZZMass<100.0&&ZZMass>=80.0)");
        else tsig[j]->Draw(cFill_sig,"MC_weight * (ZZMass<100.0&&ZZMass>=80.0 && D_bkg_kin>0.5)");
        ...
        if (!withSMDcut) tggzz[j]->Draw(cFill_ggzz, "(MC_weight)*(ZZMass<100&&ZZMass>=80)");
        else tggzz[j]->Draw(cFill_ggzz, "(MC_weight)*(ZZMass<100&&ZZMass>=80 && D_bkg_kin>0.5)");
        ...
        if (!withSMDcut){
            tqqzz[j]->Draw(cFill_qqzz, "MC_weight_ZZQQBSTU[1]*MC_weight*(ZZMass<100&&ZZMass>=80)");
        }
        else{
            tqqzz[j]->Draw(cFill_qqzz, "MC_weight_ZZQQBSTU[1]*MC_weight*(ZZMass<100&&ZZMass>=80 && D_bkg_kin>0.5)");
        }
        ...
        else tz[j]->Draw(cFill_z,"MC_weight_ZZQQBSTU[1]*MC_weight*(ZZMass<100.0&&ZZMass>=80.0 && D_bkg_kin>0.5)");
        cout << chan[j] << endl;
/*
...
*/
    ...
    string strenrichedcut = "D^{kin}_{bkg}>0.5";
...
//  hqqzz[0]->SetLineColor(TColor::GetColor("#000099"));
    ...
    TLegend *l2D;
    if(iKD!=0) l2D = new TLegend(0.20,0.57,0.58,0.90);
    else l2D = new TLegend(0.23,0.57,0.61,0.90);
    l2D->SetBorderSize(0);
    l2D->SetTextFont(42);
    ...
    char yTitle[100];
    if(plotWidth!=0.05) sprintf(yTitle,"Events / %.3f",plotWidth);
    else sprintf(yTitle,"Events / %.2f",plotWidth);

    hsig[0]->GetXaxis()->SetTitle(KDlist_title[iKD]);
    hsig[0]->GetYaxis()->SetTitle(yTitle);
    hsig[0]->GetYaxis()->SetRangeUser(0,max_data*(withSMDcut ? 1.30:1.25));
    ...
    l2D->AddEntry(hsig[0],label_BSMHypo[iKD],"l");
    ...
    l2D->Draw("same");
    pt->Draw();
    TPaveText *pt10 = new TPaveText(0.68,0.82,0.89,0.90,"brNDC");
    pt10->SetBorderSize(0);
    pt10->SetTextAlign(12);
    pt10->SetTextSize(0.05);
    pt10->SetFillStyle(0);
    pt10->SetTextFont(42);
